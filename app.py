...
app = Flask(__name__)
app.config.update(
    SECRET_KEY=os.environ.get('SMARTMETRIQ_SECRET_KEY') or secrets.token_urlsafe(32),
    SESSION_COOKIE_HTTPONLY=True,
    SESSION_COOKIE_SAMESITE='Lax',
    # Set SMARTMETRIQ_SECURE_COOKIES=1 when the app is served over HTTPS.
...
def protect_state_changing_requests():
    if request.method not in {'POST', 'PUT', 'PATCH', 'DELETE'} or request.endpoint == 'login':
        return None
    # Let route-level authentication return its normal login/401 response.
...
def handle_large_request(_error):
    if request.path == '/upload':
        return jsonify({'success': False, 'message': 'File is too large. Please upload an image under 10 MB.'}), 413
    return 'File is too large. Please upload an image under 10 MB.', 413
...
def generate_complaint_id():
    ...
    return f'CMP-{today}-{uuid.uuid4().hex[:12].upper()}'
...
def seed_demo_complaints():
    ...
    # Run on every startup so a fresh/free-hosted SQLite file always receives
    # the demo records. Existing records are left untouched and never duplicated.
    for case in demo_cases:
        if get_complaint_by_id(case['complaint_id']) is None:
...
def save_uploaded_file(file_storage, target_folder=None):
    ...
    if Image is None:
        raise RuntimeError('Image validation is unavailable. Install Pillow before accepting uploads.')
...
def generate_inspection_id():
    today = date.today().strftime('%Y%m%d')
    return f'SMQ-{today}-{uuid.uuid4().hex[:12].upper()}'
...
def is_officer():
    return session.get('user_role') == 'officer'


def can_access_inspection(inspection):
    return bool(inspection) and (
        is_officer() or inspection.get('inspector') == session.get('username')
    )


def can_access_evidence(relative_path):
    """Return whether the current user is entitled to retrieve an evidence file."""
    if is_officer():
        return True
    ...
    inspection = conn.execute(
        'SELECT 1 FROM inspections WHERE image_path = ? AND user_id = ?',
        (relative_path, username),
    ).fetchone()
    complaint = conn.execute(
        'SELECT 1 FROM complaints WHERE image_path = ? AND consumer = ?',
        (relative_path, username),
    ).fetchone()
    conn.close()
    return inspection is not None or complaint is not None
...
def uploaded_file(filename):
    ...
    if not can_access_evidence(relative_path):
        abort(403)
...
def upload_image():
    if not session.get('logged_in'):
        return jsonify({'success': False, 'message': 'Please log in before uploading an image.'}), 401
    try:
        uploaded_file = request.files.get('image')
        ...
        return jsonify({'success': False, 'message': 'An unexpected error occurred while saving the image.'}), 500
